...
export default class Render extends React.Component<{isAdmin: boolean, adminOnlyEdit: boolean}, {contentBody: string|null, inDashEditMode: boolean}>{
    constructor(props){
        super(props);
        this.state = {
            contentBody: null,
    ...
    renderContent(){
        if (this.state.contentBody) return <Typography dangerouslySetInnerHTML={{__html: this.state.contentBody}}></Typography>
    ...
    render() {
        ...
        return (
            <React.Fragment>
                {editable ?<Link to={`/edit`} style={styles.link}>
